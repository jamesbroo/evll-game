...
import { getPredictionsContractEUR } from 'utils/contractHelpers'
import {
  FUTURE_ROUND_COUNT,
...
export const fetchRound = createAsyncThunk<ReduxNodeRound, number>('predictionsEUR/fetchRound', async (epoch) => {
  const predictionContract = getPredictionsContractEUR()
  const response = await predictionContract.rounds(epoch)
  return serializePredictionsRoundsResponse(response)
})

export const fetchRounds = createAsyncThunk<{ [key: string]: ReduxNodeRound }, number[]>(
  'predictionsEUR/fetchRounds',
  async (epochs) => {
...
export const fetchMarketData = createAsyncThunk<MarketData>('predictionsEUR/fetchMarketData', async () => {
  const marketData = await getPredictionData()
  return marketData
})

export const fetchLedgerData = createAsyncThunk<LedgerData, { account: string; epochs: number[] }>(
  'predictionsEUR/fetchLedgerData',
...
>('predictionsEUR/fetchClaimableStatuses', async ({ account, epochs }) => {
  const ledgers = await getClaimStatuses(account, epochs)
  return ledgers
})

export const fetchHistory = createAsyncThunk<{ account: string; bets: Bet[] }, { account: string; claimed?: boolean }>(
  'predictionsEUR/fetchHistory',
...
>('predictionsEUR/fetchNodeHistory', async ({ account, page = 1 }) => {
...
export const filterLeaderboard = createAsyncThunk<{ results: PredictionUser[] }, { filters: LeaderboardFilter }>(
  'predictionsEUR/filterLeaderboard',
  async ({ filters }) => {
...
export const fetchAddressResult = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>('predictionsEUR/fetchAddressResult', async (account, { rejectWithValue }) => {
...
export const filterNextPageLeaderboard = createAsyncThunk<
  ...
  number,
  { state: State }
>('predictionsEUR/filterNextPageLeaderboard', async (skip, { getState }) => {
...
export const predictionsSliceEUR = createSlice({
  name: 'predictionsEUR',
  initialState,
  reducers: {
...
export const {
  ...
  setSelectedAddress,
} = predictionsSliceEUR.actions

export default predictionsSliceEUR.reducer
