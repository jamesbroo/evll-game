...
import { useGetMinBetAmount } from 'state/predictionsEUR/hooks'
import { useTranslation } from 'contexts/Localization'
import { useCake, usePredictionsContractEUR } from 'hooks/useContract'
import useTokenBalance, { useEVLLllowanceEUR, useGetBnbBalance } from 'hooks/useTokenBalance'
import useToast from 'hooks/useToast'
import { useCallWithGasPrice } from 'hooks/useCallWithGasPrice'
import { BetPosition } from 'state/types'
import { formatBigNumber, formatFixedNumber, getFullDisplayBalance } from 'utils/formatBalance'
...
import { getCakeContract, getPredictionsContractEUR } from 'utils/contractHelpers'
...
const getButtonProps = (
  ...
  cakeBalance: ethers.BigNumber,
  minBetAmountBalance: ethers.BigNumber,
) => {
  const hasSufficientBalance = () => {
    if (value.gt(0)) {
      return value.lte(cakeBalance)
    }
    return cakeBalance.gt(0)
  }

  if (!hasSufficientBalance()) {
    return { key: 'Insufficient EVLL balance', disabled: true }
  }
...
const SetPositionCard: React.FC<SetPositionCardProps> = ({ position, togglePosition, epoch, onBack, onSuccess }) => {
  ...
  const predictionsEUR = getPredictionsContractEUR()

  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await cakeContract.allowance(account, predictionsEUR.address)
      ...
      onApprove: () => {
        console.log('debug approve', cakeContract, predictionsEUR)
        return callWithGasPrice(cakeContract, 'approve', [predictionsEUR.address, ethers.constants.MaxUint256])
      },
      onConfirm: () => {
        return callWithGasPrice(cakeContract, 'approve', [predictionsEUR.address, ethers.constants.MaxUint256])
      },
      onSuccess: async ({ receipt }) => {
        console.log('debug approve on success')
      },
    })
  ...
  const currentAllowance = useEVLLllowanceEUR()

  // const { balance: cakeBalance } = useGetBnbBalance()
  const { balance: cakeBalance } = useTokenBalance(tokens.cake.address)
  const ncakeBalance = ethers.BigNumber.from(cakeBalance.toString())
  ...
  const maxBalance = useMemo(() => {
    return ncakeBalance.gt(dust)  ? ncakeBalance.sub(dust) : dust
  }, [ncakeBalance])
  const balanceDisplay = getFullDisplayBalance(cakeBalance, 18,3)
  ...
  const handleEnterPosition = async () => {
    ...
    console.log('debug enter', valueAsBn)
    try {
      
      const tx = await callWithGasPrice(predictionsContract, betMethod, [epoch, valueAsBn.toString()] )
      setIsTxPending(true)
    ...
    <Card onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
      ...
      <CardBody py="16px">
        <Flex alignItems="center" justifyContent="space-between" mb="8px">
          ...
          <Flex alignItems="center">
            {/* <BinanceIcon mr="4px  " /> */}
            <img src={logo} alt="token logo" width="20px" height="20px" />
        ...
        <Box mb="8px">
          {account ? (currentAllowance?.toString() !== '0' ? (
                <Button
                  width="100%"
