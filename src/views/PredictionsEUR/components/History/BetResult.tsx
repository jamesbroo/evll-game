...
import { fetchLedgerData, markAsCollected } from 'state/predictionsEUR'
import { Result } from 'state/predictionsEUR/helpers'
import { useGetIsClaimable } from 'state/predictionsEUR/hooks'
    ...
    <>
      ...
      <StyledBetResult>
        ...
        <Flex alignItems="center" justifyContent="space-between" mb="16px">
          ...
          <Text>{`${formatBnb(bet.amount)} EAVL`}</Text>
        </Flex>
        <Flex alignItems="start" justifyContent="space-between">
          <Text bold>{isWinner ? t('Your winnings') : t('Your Result')}:</Text>
          <Box style={{ textAlign: 'right' }}>
            <Text bold color={getResultColor()}>{`${isWinner ? '+' : '-'}${formatBnb(payout)} EAVL`}</Text>
          ...
          <>
            ...
            <Flex alignItems="start" justifyContent="space-between">
              ...
              <Flex justifyContent="end">
                <Text fontSize="14px" color="textSubtle">{`${formatBnb(returned)} EAVL`}</Text>
                <span ref={targetRef}>
