...
import { getRoundResult, Result } from 'state/predictionsEUR/helpers'
import { REWARD_RATE } from 'state/predictionsEUR/config'
import { getBscScanLink } from 'utils'
import { multiplyPriceByAmount } from 'utils/prices'
import { useBNBBusdPrice } from 'hooks/useBUSDPrice'
import { useGetCurrentEpoch } from 'state/predictionsEUR/hooks'
    ...
    <Box p="16px">
      ...
      <Flex>
        ...
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          ...
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? 'success' : 'failure'}>
            {`${netResultIsPositive ? '+' : ''}${formatBnb(netResultAmount)} EAVL`}
          </Text>
      ...
      <Box pl="8px">
        ...
        <Text bold color={avgBnbWonIsPositive ? 'success' : 'failure'}>
          {`${avgBnbWonIsPositive ? '+' : ''}${formatBnb(avgBnbWonPerRound)} EAVL`}
        </Text>
        <Text small color="textSubtle">
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="success">{`+${formatBnb(summary.won.bestRound.payout)} EAVL`}</Text>
        ...
        <Text bold>{`${formatBnb(avgPositionEntered)} EAVL`}</Text>
        <Text small color="textSubtle">
